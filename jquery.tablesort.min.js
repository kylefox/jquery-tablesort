...
h.find("tbody tr"):h.find("tr").has("td"),m=h.find("tr td:nth-of-type("+(d.index()+1)+")"),f=d.data().sortBy,k=[],l=m.map(function(c,e){return f?"function"===typeof f?f(a(d),a(e),b):f:null!=a(this).data().sortValue?a(this).data().sortValue:a(this).text()});0!==l.length&&(this.direction="asc"!==c&&"desc"!==c?"asc"===this.direction?"desc":"asc":c,c="asc"==this.direction?1:-1,b.$table.trigger("tablesort:start",[b]),b.log("Sorting by "+this.index+" "+this.direction),b.$table.css("display"),setTimeout(function(){b.$sortCells.removeClass(b.settings.asc+
" "+b.settings.desc);for(var g=0,f=l.length;g<f;g++)k.push({index:g,cell:m[g],row:n[g],value:l[g]});k.sort(function(a,b){return a.value>b.value?1*c:a.value<b.value?-1*c:0});a.each(k,function(a,b){h.append(b.row)});d.addClass(b.settings[b.direction]);b.log("Sort finished in "+((new Date).getTime()-e.getTime())+"ms");b.$table.trigger("tablesort:complete",[b]);b.$table.css("display")},10))},log:function(d){(a.tablesort.DEBUG||this.settings.debug)&&console&&console.log&&console.log("[tablesort] "+d)},
